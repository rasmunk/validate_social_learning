...
World::~World()
{
	...
	for (int i = 0; i != gNumberOfRobots; ++i) {
		if (_evaluators[i] != NULL) delete _evaluators[i];
	}

	delete _evaluatorBarrier;
...
void World::initWorld()
{
	...
	std::map<std::string, std::shared_ptr<SharedDataStructure>> sharedStorages;
	if (gRobotUseEvolution) {
		sharedStorages.insert(std::pair<std::string, std::shared_ptr<SharedDataStructure>>("gRobotUseEvolution", std::make_shared<SharedVector<Solution>>()));
	}
	if (gRobotUseSocialLearning) {
		sharedStorages.insert(std::pair<std::string, std::shared_ptr<SharedDataStructure>>("gRobotUseSocialLearning", std::make_shared<SharedStack<Solution>>()));
	}
	if (sharedStorages.size() > 0) {
		this->_evaluatorBarrier = new Barrier(gInitialNumberOfRobots);
	}
	// Initialize agents
	// Initializes the controller
	// Initialzes the evaluator
	for ( int i = 0 ; i != gInitialNumberOfRobots ; i++)
	{
		Robot *robot = new Robot(this);
        this->addRobot(robot);
		// make evaluator spawns an independent thread that monitors the passed robot controller -> closes itself upon destruction
		if (sharedStorages.size() > 0) {
			_evaluators.push_back(gConfigurationLoader->make_Evaluator_with_sharedStorage_n_barrier(robot->getController(), sharedStorages, _evaluatorBarrier));
		} else {
			_evaluators.push_back(gConfigurationLoader->make_Evaluator(robot->getController()));
		}
	}
...
void World::updateWorld(const Uint8 *__keyboardStates)
{
	...
	// update agent level observers -> current sensor values
	for ( int i = 0 ; i != gNumberOfRobots ; i++ )
...
bool World::loadFiles()
{
	...
	if (gScaleSpeed) {
		// Used in the roborobo file to readjust max transactional speed in accordance with the current FPS
		_targetPixelsPerSecond = gMaxTranslationalSpeed;
		gMaxTranslationalSpeed = (gMaxTranslationalSpeed/(1000/(1000/gFramesPerSecond)));
	}
