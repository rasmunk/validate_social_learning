...
// Neural Network
double gWeightRangeValue = 4.0;
double gDisabledWeightValue = 100.0;
...
bool gScaleSpeed = false;

double gMaxTranslationalSpeed = 2.0; // pixels/sec.
double gMaxTranslationalDeltaValue = gMaxTranslationalSpeed; // maximum delta update value btw current and desired speed.
double gMaxRotationalSpeed = 30; // degrees/sec.
int gSensorRange = 64;
int gTailLength = 16;
...
int gLocomotionMode = 0;

// Robot Settings
int gNumberOfTotalEvals = 200;
double gEvaluationSeconds = 8.75;
double gRecoverySeconds = 1.50;
int gRobotLifeTime = 100;

// Robot sigma
double gRobotInitialSigma = 1.0;
double gRobotMaxSigma = 4.0;
double gRobotMinSigma = 0.01;
double gRobotSigmaIncrease = 2.0;

//Robot Evaluation setting
double gRobotOldFitnessWeight = 0.8;
double gEvolutionVersion;
bool gRobotUseEvolution = false;
double gDisabledSensorPercentage = 0.3;
int gTournamentSize = 2;
double gMutationRate = 0.05;
int gGenomeStorageCapacity = 5;
double gMemomeSharingThreshold = 30.0;
int gMemomeStorageCapacity = 20;
bool gRobotUseSocialLearning = false;
...
{	
	...
	if ( !gBatchMode)
	{
		...
		std::cout << "=-=-=-=-=-=-=-=-=-=-=-= roborobo! =-=-=-=-=-=-=-=-=-=-=-=\n\n";
		std::cout << "Time Launched: " << std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()) << std::endl;
	}
}
...
bool loadProperties( std::string __propertiesFilename )
{
    ...
    */
	...
	if ( gProperties.hasProperty("gMaxTranslationalSpeed") )
		convertFromString<double>(gMaxTranslationalSpeed, gProperties.getProperty("gMaxTranslationalSpeed"), std::dec);
	else
	{
	...
	if ( gProperties.hasProperty("gMaxTranslationalDeltaValue") ) // ie. max update (delta) btw current speed and desired speed
		convertFromString<double>(gMaxTranslationalDeltaValue, gProperties.getProperty("gMaxTranslationalDeltaValue"), std::dec);
	else
	{
	...
	if ( gProperties.hasProperty("gMaxRotationalSpeed") )
		convertFromString<double>(gMaxRotationalSpeed, gProperties.getProperty("gMaxRotationalSpeed"), std::dec);
	else
	{
	...
	if ( gProperties.hasProperty("gWeightRangeValue")) {
		convertFromString<double>(gWeightRangeValue, gProperties.getProperty("gWeightRangeValue"), std::dec);
	}
	else
	{
		std::cerr << "[MISSING] gWeightRangeValue value is missing.\n";
		returnValue = false;
	}


	if ( gProperties.hasProperty("gDisabledWeightValue")) {
		convertFromString<double>(gDisabledWeightValue, gProperties.getProperty("gDisabledWeightValue"), std::dec);
	}
	else
	{
		std::cerr << "[MISSING] gDisabledWeightValue value is missing.\n";
		returnValue = false;
	}
	...
	s = gProperties.getProperty("gScaleSpeed");
	if ( s == "true" || s == "True" || s == "TRUE" ) {
		gScaleSpeed = true;
	}
	else {
		...
		{
			std::cerr << "[CORRUPT] gScaleSpeed should be boolean (\"true\" or \"false\").\n";
			returnValue = false;
		}
	}
	...
	if ( gProperties.hasProperty("gNumberOfTotalEvals"))
	{
		convertFromString<int>(gNumberOfTotalEvals, gProperties.getProperty("gNumberOfTotalEvals"), std::dec);
	}
	else {
		std::cerr << "[MISSING] gNumberOfTotalEvals has no string value, assumes default " << gNumberOfTotalEvals << std::endl;
	}

	if ( gProperties.hasProperty("gEvaluationSeconds"))
	{
		convertFromString<double>(gEvaluationSeconds, gProperties.getProperty("gEvaluationSeconds"), std::dec);
	} else {
		std::cerr << "[MISSING] gEvaluationSeconds has no string value, assumes default " << gEvaluationSeconds << std::endl;
	}


	if ( gProperties.hasProperty("gRecoverySeconds"))
	{
		convertFromString<double>(gRecoverySeconds, gProperties.getProperty("gRecoverySeconds"), std::dec);
	} else {
		std::cerr << "[MISSING] gRecoverySeconds has no string value, assumes default " << gRecoverySeconds << std::endl;
	}



	if ( gProperties.hasProperty("gRobotLifeTime"))
	{
		convertFromString<int>(gRobotLifeTime,gProperties.getProperty("gRobotLifeTime"),std::dec);
	}
	else {
		std::cerr << "[MISSING] gRobotLifeTime has no string value, assumes default " << gRobotLifeTime << std::endl;
	}

	if ( gProperties.hasProperty("gRobotInitialSigma"))
	{
		convertFromString<double>(gRobotInitialSigma, gProperties.getProperty("gRobotInitialSigma"), std::dec);
	}
	else {
		std::cerr << "[MISSING] gRobotInitialSigma has no string value, assumes default " << gRobotInitialSigma << std::endl;
	}
	...
	if ( gProperties.hasProperty("gRobotMaxSigma"))
	{
		convertFromString<double>(gRobotMaxSigma, gProperties.getProperty("gRobotMaxSigma"), std::dec);
	}
	else {
		std::cerr << "[MISSING] gRobotMaxSigma has no string value, assumes default " << gRobotMaxSigma << std::endl;
	}

	if ( gProperties.hasProperty("gRobotMinSigma"))
	{
		convertFromString<double>(gRobotMinSigma,gProperties.getProperty("gRobotMinSigma"), std::dec);
	}
	else {
		std::cerr << "[MISSING] gRobotMinSigma has no string value, assumes default " << gRobotMinSigma << std::endl;
	}

	if ( gProperties.hasProperty("gRobotSigmaIncrease"))
	{
		convertFromString<double>(gRobotSigmaIncrease, gProperties.getProperty("gRobotSigmaIncrease"), std::dec);
	}
	else {
		std::cerr << "[MISSING] gRobotSigmaIncrease has no string value, assumes default " << gRobotSigmaIncrease << std::endl;
	}

	if ( gProperties.hasProperty("gRobotOldFitnessWeight"))
	{
		convertFromString<double>(gRobotOldFitnessWeight, gProperties.getProperty("gRobotOldFitnessWeight"), std::dec);
	}
	else {
		std::cerr << "[MISSING] gRobotOldFitnessWeight has no string value, assumes default " << gRobotOldFitnessWeight << std::endl;
	}

	if ( gProperties.hasProperty("gEvolutionVersion"))
	{
		convertFromString<double>(gEvolutionVersion, gProperties.getProperty("gEvolutionVersion"), std::dec);
	}
	else {
		std::cerr << "[MISSING] gEvolutionVersion has no string value, assumes default " << gEvolutionVersion << std::endl;
	}

	s = gProperties.getProperty("gRobotUseEvolution");
	if ( s == "true" || s == "True" || s == "TRUE" )
		gRobotUseEvolution = true;
	else
	if ( s == "false" || s == "False" || s == "FALSE" )
		gRobotUseEvolution = false;
	else
	{
		std::cerr << "[CORRUPT] gRobotUseEvolution should be boolean (\"true\" or \"false\").\n";
		returnValue = false;
	}

	if ( gProperties.hasProperty("gTournamentSize"))
	{
		convertFromString<int>(gTournamentSize, gProperties.getProperty("gTournamentSize"), std::dec);
	}
	else {
		std::cerr << "[MISSING] gTournamentSize has no string value, assumes default " << gTournamentSize << std::endl;
	}

	if ( gProperties.hasProperty("gMutationRate"))
	{
		convertFromString<double>(gMutationRate, gProperties.getProperty("gMutationRate"), std::dec);
	}
	else {
		std::cerr << "[MISSING] gMutationRate has no string value, assumes default " << gMutationRate << std::endl;
	}

	if ( gProperties.hasProperty("gGenomeStorageCapacity"))
	{
		convertFromString<int>(gGenomeStorageCapacity, gProperties.getProperty("gGenomeStorageCapacity"), std::dec);
	}
	else {
		std::cerr << "[MISSING] gGenomeStorageCapacity has no string value, assumes default " << gGenomeStorageCapacity << std::endl;
	}

	if ( gProperties.hasProperty("gMemomeSharingThreshold"))
	{
		convertFromString<double>(gMemomeSharingThreshold, gProperties.getProperty("gMemomeSharingThreshold"), std::dec);
	}
	else {
		std::cerr << "[MISSING] gMemomeSharingThreshold has no string value, assumes default " << gMemomeSharingThreshold << std::endl;
	}

	if ( gProperties.hasProperty("gMemomeStorageCapacity"))
	{
		convertFromString<int>(gMemomeStorageCapacity, gProperties.getProperty("gMemomeStorageCapacity"), std::dec);
	}
	else {
		std::cerr << "[MISSING] gMemomeStorageCapacity has no string value, assumes default " << gMemomeStorageCapacity << std::endl;
	}



	s = gProperties.getProperty("gRobotUseSocialLearning");
	if ( s == "true" || s == "True" || s == "TRUE" )
		gRobotUseSocialLearning = true;
	else
	if ( s == "false" || s == "False" || s == "FALSE" )
		gRobotUseSocialLearning = false;
	else
	{
		std::cerr << "[CORRUPT] gRobotUseSocialLearning should be boolean (\"true\" or \"false\").\n";
		returnValue = false;
	}
...
bool runRoborobo()
{
	...
	bool quit = false;
	std::vector<bool> finshedEvaluators;
	int fpsTimesDifferent = 0;
	float currentFps = 0.0;
	while(!quit)
	{
		//Start the frame timer
		if ( gBatchMode ) {
			// change to include epoch
			fps.start();
			gWorld->updateWorld();

			// Limit the amount of distance the robots can move per second
			if( fps.get_ticks() < (1000 / gFramesPerSecond) )
			{
				SDL_Delay( (1000 / gFramesPerSecond) - fps.get_ticks());
			}
			currentFps  = (1000/fps.get_ticks());
		...
		// Readjust gMaxTranslationalspeed if the framerate is less than the expected -> robots should have a maximum speed of 14cm/s
		if (gScaleSpeed && (currentFps < gFramesPerSecond)) {
			fpsTimesDifferent++;
			if (fpsTimesDifferent > 50) { // can't achive the fps -> limit it
				gMaxTranslationalSpeed = (gWorld->getTargetPixelsPerSecond()/currentFps);
				gFramesPerSecond = (int)currentFps;
				std::cout << "Max FPS changed to: " << gFramesPerSecond << std::endl;
				std::cout << "Translationalspeed: " << gMaxTranslationalSpeed << std::endl;
				fpsTimesDifferent = 0;
			}
		} else {
			fpsTimesDifferent = 0;
		...
		finshedEvaluators.clear();
		// Quit if all evaluators have completed
		for (int i = 0; i<gWorld->_evaluators.size(); ++i) {
			if (gWorld->_evaluators[i]->finished()) {
				finshedEvaluators.push_back(true);
				std::cout << "Pushing back finshed size: " << finshedEvaluators.size() << " Evaluators size: " << gWorld->_evaluators.size() << std::endl;
			}
		}
		if (finshedEvaluators.size() == gWorld->_evaluators.size()) {
			quit = true;
		}
